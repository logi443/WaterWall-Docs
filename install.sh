...
install_waterwall() {
	LATEST_RELEASE=$(curl --silent "https://github.com/radkesvat/WaterWall/releases/download/v1.32" | grep -Po '"tag_name": "\K.*?(?=")')
	...
	if [ ! -f "$INSTALL_DIR/$FILE_NAME" ]; then
		...
		if [ "$ARCH" == "x86_64" ]; then
			...
			DOWNLOAD_URL="https://github.com/radkesvat/WaterWall/releases/download/v1.32/Waterwall-linux-arm64.zip"
		else
